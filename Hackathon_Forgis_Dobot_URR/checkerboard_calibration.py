...
def process_images_from_dir(image_dir, checkerboard_size, square_size, show_images=True):
    """Loads images from a path and runs calibration."""
    ...
    for fname in images:
        ...
        if ret:
            ...
            if show_images:
                ...
                h, w = img.shape[:2]
                display_img = cv2.resize(img, (int(w * 0.5), int(h * 0.5))) if w > 1200 else img
                cv2.imshow('Checkerboard Detection', display_img)
                cv2.waitKey(500)
        else:
            print(f"Warning: Could not find '{cols}x{rows}' checkerboard corners in {fname}")
...
def process_images_from_camera(camera_id, checkerboard_size, square_size):
    ...
    print(" - Press 'c' to run calibration on captured frames.")
...
if __name__ == "__main__":
    ...
    if args.camera is not None:
        ...
        process_images_from_dir(args.image_dir, checkerboard_dims, args.size, not args.no_display)
    else:
        print("Please provide either --image_dir or --camera argument. Use -h for help.")
